...
public class MysqlService {
    ...
    private static Map<String, MysqlService> mysqlManagers = new HashMap<String, MysqlService>();
    private DataSourceManager dataSourceManager;

    private CSLogger logger = Loggers.getLogger(MysqlService.class);

    private Settings settings;

    @Inject
    public MysqlService(DataSourceManager _dataSourceManager, Settings _settings) {
        this.settings = _settings;
        for (Map.Entry<String, DataSource> entry : _dataSourceManager.dataSourceMap().entrySet()) {
            mysqlManagers.put(entry.getKey(), new MysqlService(_dataSourceManager, settings, entry.getValue()));
        }

    }

    private MysqlService(DataSourceManager dataSourceManager, Settings settings, DataSource _dataSource) {
        this.settings = settings;
        this.dataSource = _dataSource;
        this.dataSourceManager = dataSourceManager;
    }

    public MysqlService mysqlService(String dataSourceName) {
        return mysqlManagers.get(dataSourceName);
    }

    public MysqlService defaultMysqlService() {
    ...
    public <T> Set<T> projection_query_as_set(String sql, final String columnName, Object... objs) {
        return (Set<T>) defaultMysqlService().executeQuery(sql, new MysqlService.SqlCallback() {
            @Override
    ...
    public <T> List<T> projection_query(String sql, final String columnName, Object... objs) {
        return (List<T>) defaultMysqlService().executeQuery(sql, new MysqlService.SqlCallback() {
            @Override
            public Object execute(ResultSet rs) {
    ...
    public List<Map> query(String sql, Object... objs) {
        return (List<Map>) defaultMysqlService().executeQuery(sql, new MysqlService.SqlCallback() {
            @Override
            public Object execute(ResultSet rs) {
                return MysqlService.rsToMaps(rs);
            }
        }, objs);
    }

    public Map single_query(String sql, Object... objs) {
        return (Map) defaultMysqlService().executeQuery(sql, new MysqlService.SqlCallback() {
            @Override
            public Object execute(ResultSet rs) {
                try {
                    return MysqlService.rsToMapSingle(rs, MysqlService.getRsCloumns(rs));
                } catch (SQLException e) {
                    e.printStackTrace();
                }
                return null;
